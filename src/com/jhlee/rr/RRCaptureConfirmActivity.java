...
public class RRCaptureConfirmActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 *  Rotate bitmap
		 **/
		Button.OnClickListener btnRotationListener= new Button.OnClickListener() {
			public void onClick(View v) {
				Matrix m =imgView.getImageMatrix();
				
				int vw = imgView.getWidth();
				int vh = imgView.getHeight();
				m.postTranslate(-vw/2, -vh/2);
				m.postRotate(90.0f);
				m.postTranslate(vw/2, vh/2);
				
				/*
				Bitmap newBmp = Bitmap.createBitmap(mCapturedBmp, 0, 0, 
						mCapturedBmp.getWidth(),
						mCapturedBmp.getHeight(),
						rotateMatrix,
						false);
						*/
				//imgView.setImageMatrix(rotateMatrix);
				imgView.invalidate();
			}
		};
		Button btnRotation = (Button) findViewById(R.id.button_rotate);
		btnRotation.setOnClickListener(btnRotationListener);
	}
