 ...
 * 
 */
public class RRTopicListView extends View {
	...
	/** CTOR */
	public RRTopicListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initializeInternal(attrs);
	}

	public RRTopicListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initializeInternal(attrs);
	}

	public RRTopicListView(Context context) {
		super(context);
	}
	...
	private Bitmap createTextBlurBitmap(String text, Paint paint,
		...
		/* Set up blur filter */
		MaskFilter blur = new BlurMaskFilter(3.5f, BlurMaskFilter.Blur.NORMAL);
		...
		case MotionEvent.ACTION_MOVE:
			if (mMouseDownFlag == true) {
				...
				mMouseMaxMovement = Math.max(mMouseMaxMovement, Math.abs(y
						- mMouseDownY));
				if (mMouseMaxMovement > (mItemHeight>>1)) {
					mFocusItemIndex = -1;
				}
				invalidate();
			}
	...
	private void startAlignAnimation() {
		final RRTopicListView view = this;
		if (mAlignTask == null) {
